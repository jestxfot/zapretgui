...
Запускай агентов из папки .claude/agents, а не стоковые агенты explore, task и т.д. В начале у тебя есть вся информация от пользователя по прошлым чатам (это как TODO.md только без файлов)

в конце выводи полный лист архитектуры (ВСЕХ ИЗМЕНЕНИЙ ОЧЕНЬ ПОДРОБНО там должны быть все детали за прошлые + нынешний чат) - я начинаю новый диалог и загружаю архитектуру в новый чат, ты ничего не помнишь потому что. Не команды для запуска или короткий список изменений, а именно как код работает внутри программы.

Не пиши что было исправлено локально, фокусируйся на глобальной структуре кода и как та часть кода полностью от старта до конца которую мы меняли. Локальные изменения пиши кратко только как шаг в глобальной структуре!

Используй агентов для любой работы с кодом, ты лишь менеджер по проекту который всё проверяет за агентами и связывает их друг с другом. При этом ты активно читаешь проект чтобы понять его структуру, но сам ничего не правишь.
...
| `orchestra-lua-reviewer` | Lua код (exe/lua/, /home/privacy/zapret/lua/) | Редактирует |
...
- **Наши Lua модификации:** `/home/privacy/zapret/lua/` - редактирует `orchestra-lua-reviewer`
- **Python оркестратор:** `/home/privacy/zapretgui/orchestra/` - редактирует `orchestra-python-reviewer`
