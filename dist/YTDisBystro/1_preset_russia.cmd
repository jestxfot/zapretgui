...
set YTDB_RTMPS=43 --ipset="%~dp0lists\russia-youtube-rtmps.txt" --dpi-desync=syndata --dpi-desync-fake-syndata="%~dp0fake\tls_clienthello_4.bin" --dpi-desync-fooling=badseq
...
--filter-tcp=80,443 --hostlist="%~dp0lists\mycdnlist.txt" --dpi-desync=fakeddisorder --dpi-desync-split-pos=2,midsld --dpi-desync-fakedsplit-pattern="%~dp0fake\tls_clienthello_1.bin" --dpi-desync-fooling=badseq --new ^
--filter-tcp=80 --hostlist="%~dp0lists\russia-blacklist.txt" --hostlist="%~dp0lists\myhostlist.txt" --dpi-desync=fake,multisplit --dpi-desync-split-seqovl=1 --dpi-desync-split-pos=sld+1 --dpi-desync-fooling=badseq --new ^
--filter-tcp=443 --hostlist="%~dp0lists\russia-blacklist.txt" --hostlist="%~dp0lists\myhostlist.txt" --dpi-desync=fakedsplit --dpi-desync-fooling=badseq --dpi-desync-split-pos=2,midsld-1 --dpi-desync-fakedsplit-pattern="%~dp0fake\tls_clienthello_4.bin" --new ^
--filter-tcp=443 %YTDB_DIS1% --new ^
--filter-udp=443 %YTDB_DIS2% --new ^
--filter-udp=50000-50090 %YTDB_DIS3% --new ^
--filter-tcp=4%YTDB_ZMESS% --new ^
