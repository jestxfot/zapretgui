...
start "zapret: winws custom 6" /b "winws.exe" ^
 ...
 --filter-tcp=443 --dpi-desync=fake,split2 --hostlist-exclude-domains=lmarena.ai,searchengines.guru,habr.com,cdn.ampproject.org,st.top100.ru,rootsplants.co.uk,podviliepitomnik.ru,cvetovod.by,veresk.by,use.fontawesome.com,xn--p1ai --dpi-desync-repeats=6 --dpi-desync-fooling=md5sig --dpi-desync-fake-tls="tls_clienthello_www_google_com.bin" --new ^
