@echo off
REM Стратегия Discord Fake Split
REM VERSION: 1.0
...
setlocal
set "BIN=%~dp0"
set "vbsSilent=%BIN%runsilent_Discord_fake_and_split.vbs"
> "%vbsSilent%" (
    ...
    echo cmd = cmd ^& " --filter-udp=443 --hostlist=""discord.txt"" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fake-quic=""quic_initial_www_google_com.bin"" --new"
    echo cmd = cmd ^& " --filter-udp=50000-50010 --ipset=""ipset-discord.txt"" --dpi-desync=fake --dpi-desync-any-protocol --dpi-desync-cutoff=d3 --dpi-desync-repeats=6 --new"
    ...
    echo cmd = cmd ^& " --filter-tcp=443 --hostlist=""discord.txt"" --dpi-desync=fake,split --dpi-desync-repeats=6 --dpi-desync-fooling=badseq --dpi-desync-fake-tls=""tls_clienthello_www_google_com.bin"" --new"
