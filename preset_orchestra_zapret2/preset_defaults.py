...
def _get_templates_dir() -> Path:
    """Returns path to orchestra preset templates directory."""
    try:
        from config import get_zapret_orchestra_presets_template_dir
        return Path(get_zapret_orchestra_presets_template_dir())
    except Exception:
        return Path("")
...
def _get_deleted_presets_ini_path() -> Path:
    """Path to deleted_presets.ini inside orchestra presets directory."""
    try:
        from config import get_zapret_orchestra_presets_dir
        return Path(get_zapret_orchestra_presets_dir()) / "deleted_presets.ini"
    except Exception:
        return Path("")
...
def ensure_templates_copied_to_presets() -> bool:
    ...
    try:
        ...
        presets_dir = Path(get_zapret_orchestra_presets_dir())
...
def overwrite_templates_to_presets() -> tuple[int, int, list[str]]:
    ...
    """
    try:
        from config import get_zapret_orchestra_presets_template_dir, get_zapret_orchestra_presets_dir
    except Exception:
        return (0, 0, [])
