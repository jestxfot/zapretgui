...
class ThemeSubscriptionManager:
    ...
    def update_proxy_button_state(self, is_active: bool = None):
        ...
        Теперь использует новый интерфейс NetworkPage.
        ...
        try:
            ...
            # is_blocked = НЕ is_active (если доступ активен, значит НЕ заблокирован)
            self.network_page.update_proxy_status(is_blocked=not is_active)
            from log import log
            log(f"Статус proxy обновлён: {'доступ включён' if is_active else 'доступ отключён'}", "DEBUG")
        except Exception as e:
            from log import log
            log(f"Ошибка при обновлении статуса proxy: {e}", "WARNING")
    ...
    def set_proxy_button_loading(self, is_loading: bool, text: str = ""):
        ...
        try:
            ...
            btn = self.network_page.proxy_toggle_btn
            
            if is_loading:
                loading_text = text if text else "Обработка..."
                btn.setText(loading_text)
                btn.setEnabled(False)
                btn.setIcon(qta.icon('fa5s.spinner', color='white'))
            else:
                btn.setEnabled(True)
            ...
            from log import log
            log(f"Ошибка в set_proxy_button_loading: {e}", "WARNING")