...
class Zapret1DirectStrategiesPage(QWidget):
    ...
    def _load_content(self):
        ...
        try:
            ...
            if mode != "direct_zapret1":
                log(f"Запрошен режим {mode}, страница рассчитана на direct_zapret1", "DEBUG")
    ...
    def _on_strategy_item_clicked(self, category_key: str, strategy_id: str):
        ...
        try:
            ...
            # Создаем объект для запуска Direct режима (Запрет 1)
            combined_data = {
                'id': 'DIRECT_MODE',
                'name': 'Прямой запуск (Запрет 1)',
                'is_combined': True,
            ...
            if hasattr(app, 'dpi_controller') and app.dpi_controller:
                ...
                # Обновляем UI
                if hasattr(app, 'current_strategy_label'):
                    app.current_strategy_label.setText("Прямой запуск (Запрет 1)")
                if hasattr(app, 'current_strategy_name'):
                    app.current_strategy_name = "Прямой запуск (Запрет 1)"
            ...
            self.strategy_selected.emit("combined", "Прямой запуск (Запрет 1)")
    ...
    def _start_process_monitoring(self):
        """Запускает мониторинг статуса процесса winws"""
        self._process_check_attempts = 0
    ...
    def _check_process_status(self):
        """Проверяет реальный статус процесса winws"""
        try:
