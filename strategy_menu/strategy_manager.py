# strategy_menu/strategy_manager.py
...
class StrategyManager:
    ...
    ВНИМАНИЕ: модуль не содержит побочных эффектов при импорте.
    """
    ...
    def __init__(self, local_dir: str, json_dir: str, status_callback=None, **kwargs):
        ...
        # Настройки источников и ретраев (могут использоваться, даже если загрузка отключена)
        self.url_sources = URL_SOURCES
        self.failed_sources: set[int] = set()
        self.max_retries = 3
        self.retry_delay = 2.0
        self.current_source_index = 0
        self.last_update_time: float | None = None

        # Режим только локальных стратегий (по умолчанию True, можно переопределить снаружи)
    ...
    def save_strategies_index(self, data: dict = None) -> bool:
        ...
        try:
            ...
            with open(os.path.join(self.json_dir, "index.json"), "w", encoding="utf-8-sig") as f:
    ...
    # ───────────────────────── опционально: сеть (если включите в будущем) ─────
    ...
    def _download_strategies_index(self) -> dict:
        """Скачивание index.json с резервными источниками (если включите интернет)."""
        ...
        for source_index, source in enumerate(self.url_sources):
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    response = session.get(index_url, timeout=(15, 45), stream=False)
    ...
    def _download_strategy_sync(self, strategy_id: str) -> str | None:
        """Синхронная загрузка конкретной стратегии (если включите интернет)."""
        local_path = os.path.join(self.local_dir, f"{strategy_id}.bat")
        try:
            ...
            for source_index, source in enumerate(self.url_sources):
                ...
                for attempt in range(self.max_retries):
                    try:
                        ...
                        response = session.get(url, timeout=(15, 45), stream=True)
                        response.raise_for_status()
    ...
    def save_strategy_version(self, file_path, info) -> bool:
        try:
            sid = next((k for k, v in self.strategies_cache.items() if v == info), None)
            if not sid:
                return False
    ...
    def check_strategy_version_status(self, strategy_id: str, strategies_cache: dict = None) -> str:
        """Проверяет статус локальной стратегии: current | not_downloaded | unknown."""
        try:
    ...
    def check_sources_availability(self) -> dict:
        """Проверяет доступность всех источников (если интернет будет включён)."""
        results = {}
