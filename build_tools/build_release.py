...
# ПО УМОЛЧАНИЮ УВЕЛИЧИВАЕТСЯ НА ОДИН И НЕ ЗАПРАШИВАЕТСЯ ПОДТВЕРЖДЕНИЕ ЧТОБЫ БЫСТРО УВЕЛИЧИТЬ НОМЕР ВЕРСИИ НА ОДИН 
...
def patch_version_json(channel: str, version: str, url: str, notes: str):
    ...
    Если скачать оригинал не удалось – создаём «с нуля»,
    но предварительно делаем резервную копию на сервере.
    """
    ...
    # ────────────────────────────────────────────────────────────
    # 1. Пытаемся получить актуальный JSON
    # ────────────────────────────────────────────────────────────
    data = {}
    got  = False
    ...
    # б) requests (помогает, если проблема только в schannel)
    if not got:
        try:
            import requests
    ...
    # в) ssh cat (берём файл напрямую с сервера по ssh)
    if not got:
        try:
            content = run(["ssh", "-i", str(SSH_KEY), SSH_HOST,
                           f"cat {REMOTE_DIR}/version.json"],
    ...
    # если ни один способ не сработал – продолжим с пустым словарём
    if not got:
        print("[WARN] не удалось получить исходный version.json – "
              "создам новый файл только с текущим каналом")
    ...
    print("✔ version.json patched & uploaded (backup on server)")
