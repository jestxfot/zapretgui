...
start "zapret: winws Rostelecom_MGTS" /b "winws.exe" ^
 ...
 --filter-tcp=443 --hostlist="discord.txt" --hostlist="other.txt" --hostlist="faceinsta.txt" --ipset="ipset-cloudflare.txt" --dpi-desync=fake,split --dpi-desync-autottl=2 --dpi-desync-repeats=6 --dpi-desync-fooling=badseq --dpi-desync-fake-tls="tls_clienthello_www_google_com.bin" --new