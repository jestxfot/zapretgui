...
setlocal
set "BIN=%~dp0"
set "vbsSilent=%BIN%runsilent_Discord_TCP_80.vbs"
> "%vbsSilent%" (
    ...
    echo cmd = cmd ^& " --filter-udp=443 --hostlist=""youtubeQ.txt"" --dpi-desync=fake --dpi-desync-repeats=2 --dpi-desync-cutoff=n2 --dpi-desync-fake-quic=""quic_test_00.bin"" --new"
    ...
    echo cmd = cmd ^& " --filter-tcp=443 --hostlist=""youtube.txt"" --dpi-desync=fake,split2 --dpi-desync-split-seqovl=2 --dpi-desync-split-pos=3 --dpi-desync-fake-tls=""tls_clienthello_www_google_com.bin"" --dpi-desync-ttl=3 --new"
    echo cmd = cmd ^& " --filter-tcp=80 --hostlist=""discord.txt"" --dpi-desync=fake,split2 --dpi-desync-autottl=2 --dpi-desync-fooling=md5sig --new"
    echo cmd = cmd ^& " --filter-udp=443 --hostlist=""discord.txt"" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fake-quic=""quic_1.bin"" --new"
    echo cmd = cmd ^& " --filter-udp=50000-59000 --dpi-desync=fake --dpi-desync-any-protocol --dpi-desync-cutoff=d3 --dpi-desync-repeats=6 --new"
    echo cmd = cmd ^& " --filter-tcp=443 --hostlist=""discord.txt"" --dpi-desync=split2 --dpi-desync-split-seqovl=652 --dpi-desync-split-pos=2 --dpi-desync-split-seqovl-pattern=""tls_clienthello_4.bin"" --new"
