@echo off
REM Стратегия datanoack padencap midsld (20.03.2025)
REM VERSION: 1.0
...
setlocal
set "BIN=%~dp0"
set "vbsSilent=%BIN%runsilent_datanoack_padencap_midsld.vbs"
> "%vbsSilent%" (
    ...
    echo cmd = cmd ^& " --filter-tcp=443 --hostlist=""discord.txt"" --ipset=""ipset-cloudflare.txt"" --dpi-desync=fake,fakeddisorder --dpi-desync-fooling=datanoack --dpi-desync-split-pos=midsld --dpi-desync-fake-tls-mod=rnd,rndsni,padencap"
