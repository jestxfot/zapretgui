...
### НЕЛЬЗЯ:
- Читать `/opt/zapretgui/*` - это код проекта, НЕ исходный код
- Читать `/opt/zapret/lua/*` - это МОДИФИЦИРОВАННЫЕ копии, НЕ оригинал
- Редактировать ЛЮБЫЕ файлы
- Писать код

### ИСКЛЮЧЕНИЯ - НЕ ЧИТАТЬ:
- `/opt/zapret/lua/zapret-auto.lua` - автообновляемая копия, читай оригинал из F:\doc\zapret2
