...
1. **КОНТРАКТ** - Какой контракт у функции? Входы, выходы, побочные эффекты?
2. **ЗАВИСИМОСТИ** - Кто вызывает эту функцию? Что она вызывает?
3. **ТИПЫ** - Какие типы ожидаются? Что если придёт None/пустой список?
4. **ИНТЕГРАЦИЯ** - Как это связано с Lua? Какие логи парсятся/генерируются?
5. **ТЕСТИРОВАНИЕ** - Как проверить что изменение работает?

**Не торопись!** Ошибка в парсере логов сломает всё обучение стратегий.
